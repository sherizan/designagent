...
async function initFromPreset(targetPath, preset) {
    // Create directories
    ...
    await writeFile(join(targetPath, 'system.json'), JSON.stringify(systemJson, null, 2), 'utf-8');
    // Write tokens/core.json
    await writeFile(join(targetPath, 'tokens', 'core.json'), JSON.stringify(preset.tokens.core, null, 2), 'utf-8');
    // Write tokens/semantic.light.json
    await writeFile(join(targetPath, 'tokens', 'semantic.light.json'), JSON.stringify(preset.tokens.semantic.light, null, 2), 'utf-8');
    // Write tokens/semantic.dark.json if present
    if (preset.tokens.semantic.dark) {
        await writeFile(join(targetPath, 'tokens', 'semantic.dark.json'), JSON.stringify(preset.tokens.semantic.dark, null, 2), 'utf-8');
    }
    // Write platform rules
    await writeFile(join(targetPath, 'tokens', 'platform.web.json'), JSON.stringify(preset.tokens.platformRules.web, null, 2), 'utf-8');
    await writeFile(join(targetPath, 'tokens', 'platform.rn.json'), JSON.stringify(preset.tokens.platformRules.rn, null, 2), 'utf-8');
    // Write contracts
    for (const contract of preset.contracts) {
        await writeFile(join(targetPath, 'contracts', `${contract.name}.contract.json`), JSON.stringify(contract.content, null, 2), 'utf-8');
    }
