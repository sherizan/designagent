...
export class TokenCompiler {
  ...
  compile(options: CompileOptions): CompiledTokens {
    ...
    for (const [tokenName, semanticToken] of Object.entries(semanticTokens.tokens)) {
      ...
      compiled[tokenName] = this.transformValue(
        coreToken.value,
        coreToken.category,
        categoryTransforms
      );
  ...
  private transformValue(
    ...
    categoryTransforms: Map<string, string>
  ): string | number {
 ...
 */
export function createTokenCompiler(): TokenCompiler {
 ...
 */
export function compileTokens(options: CompileOptions): CompiledTokens {
